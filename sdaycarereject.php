...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['id'])) {
    ...
    echo '<section class="py-5">
        <div class="container px-4 px-lg-5 mt-3">
            <div class="row">
                <div class="col">
                    <h1>Reject Daycare Booking</h1>
                </div>
            </div>';

    // Form for rejection reason
    echo '<form action="sdaycarereject.php" method="post">';
    echo '<input type="hidden" name="booking_id" value="' . $booking_id . '">';

    // Reason input
    echo '<div class="form-group">';
    echo '<label for="reason">Reason for rejection:</label>';
    echo '<textarea class="form-control" id="reason" name="reason" rows="4" required></textarea>';
    echo '</div>';
    ...
    // Redirect back to sdaycaredetail.php if booking ID is not provided or invalid request
    header('Location: sdaycaredetail.php');
    exit();
}
