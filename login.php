...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_staff = "SELECT id, FirstName, LastName, Password FROM staff WHERE Email = ?";
    if ($stmt_staff = $conn->prepare($sql_staff)) {
        ...
        if ($stmt_staff->execute()) {
            ...
            if ($stmt_staff->num_rows == 1) {
                // Bind result variables
                $stmt_staff->bind_result($id, $FirstName, $LastName, $hashed_password);
                if ($stmt_staff->fetch()) {
                    // Verify password
                    if (password_verify($password, $hashed_password)) {
                        ...
                        // Redirect staff user to staff homepage
                ...
                // Display an error message if email doesn't exist for staff
                $email_err = "No staff account found with that email.";
            }
        } else {
            echo "Oops! Something went wrong. Please try again later.";
        }
